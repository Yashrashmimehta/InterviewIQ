 ...
 const {userData} = useSelector((state)=>state.user)
  ...
  return (
    <div className='bg-[#f3f3f3] flex justify-center px-4 pt-6'>
        ...
        className='w-full max-w-6xl bg-white rounded-[24px] shadow-sm border border-gray-200 px-8 py-4 flex justify-between items-center relative border-red-500'>
            {/* Parent div for robot and title */}
            <motion.div 
                     ...
                     {showUserPopup && (
                        <div className='absolute right-0 mt-3 w-48 bg-white shadow-xl border border-gray-200 rounded-xl p-4 z-50'>
                            <p className='text-md text-blue-500
                             font-medium mb-1'>{userData?.name}</p>

                            <button onClick={()=>navigate("/history")} className='w-full text-left text-sm py-3 hover:text-blue-500 text-gray-800'>InterView History</button>
